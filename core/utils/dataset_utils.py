...
def load_detections_into_dataset(
    ...
    for i, record_ori in enumerate(dataset_dicts):
        ...
        # # Count name of object in every image in dataset_dicts
        obj_annotations = {}
        for anno in dataset_dicts[i]["annotations"]:
            obj_id = anno["category_id"]
            obj_name = data_ref.objects[obj_id]
            obj_annotations[obj_name] = []
        ...
        for det in dets_i:
            ...
            # regardless of the object that not in the dataset_dicts
            if obj_name not in obj_annotations:
                continue
...
def my_build_batch_data_loader(
    ...
    *,
    ...
    prefetch_factor = int(total_batch_size/num_workers*5)
    assert prefetch_factor > 0, "prefetch_factor should be greater than 0"
    logger.info("batch for every gpu {}".format(batch_size))
