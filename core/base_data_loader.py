...
class Base_DatasetFromList(data.Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.img_format = try_get_key(cfg, "INPUT.FORMAT", "img_format")  
    ...
    def normalize_image(self, cfg, image, pixel_mean, pixel_std):
    ...
    def aug_bbox_non_square(self, cfg, bbox_xyxy, im_H, im_W):
        ...
        return bbox_auged
    def aug_bbox_DZI(self, cfg, bbox_xyxy, im_H, im_W, dzi_type, dzi_pad_scale, dzi_scale_ratio, dzi_shift_ratio):
        x1, y1, x2, y2 = bbox_xyxy.copy()
        cx = 0.5 * (x1 + x2)
        cy = 0.5 * (y1 + y2)
        bh = y2 - y1
