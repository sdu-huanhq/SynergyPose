...
class GDRN_Online_DatasetFromList(Base_DatasetFromList):
    ...
    def __init__(self, cfg, split, lst: list, copy: bool = True, serialize: bool = True, flatten=True):
        ...
        # NOTE: process image
        self.pixel_mean = np.array(try_get_key(cfg, "MODEL.PIXEL_MEAN", "pixel_mean")).reshape(-1, 1, 1)
        self.pixel_std = np.array(try_get_key(cfg, "MODEL.PIXEL_STD", "pixel_std")).reshape(-1, 1, 1)
        self.dzi_type = try_get_key(cfg, "INPUT.DZI_TYPE", "dzi_type").lower()
        self.dzi_pad_scale = try_get_key(cfg, "INPUT.DZI_PAD_SCALE", "dzi_pad_scale")
        self.dzi_scale_ratio = try_get_key(cfg, "INPUT.DZI_SCALE_RATIO", "dzi_scale_ratio") # 0.25
        self.dzi_shift_ratio = try_get_key(cfg, "INPUT.DZI_SHIFT_RATIO", "dzi_shift_ratio") # 0.25
        ...
        self._lst = flat_dataset_dicts(lst) if flatten else lst # source dataset_dict is annotated on a single image basis, new dataset_dict is annotated on a roi
        # ----------------------------------------------------
        self._copy = copy
        self._serialize = serialize

        def _serialize(data):
            buffer = pickle.dumps(data, protocol=-1) # Serializes the obj and returns a binary object
            return np.frombuffer(buffer, dtype=np.uint8) # frombuffer can convert binary to decimal

        if self._serialize:
            # self._lst = self._lst[:50]
            logger.info("Serializing {} elements to byte tensors and concatenating them all ...".format(len(self._lst)))
            self._lst = [_serialize(x) for x in self._lst]
            self._addr = np.asarray([len(x) for x in self._lst], dtype=np.int64) # Calculate the length of each roi data (binary format)
            
            # Sum up all the data, The number at a location is the sum of the number at that location and all the previous numbers
            # 相当于是计算每条roi数据的初始地址，每条roi数据的地址为初始地址+该数据的长度
            # self.roi_addr = self._addr[idx]:self._addr[idx]+len(self._lst[idx])
            self._addr = np.cumsum(self._addr) # addr的长度等于数据的数量
            self._lst = np.concatenate(self._lst) # 将所有roi数据放到一个列表中（二进制格式），只有一个列表，列表内没有子列表
    ...
    def _get_extents(self, dataset_name):
        ...
        return self.extents[dataset_name] # extents equel to size_x/y/z in model_info.json
    ...
    def read_data_train(self, dataset_dict):
        ...
        image = read_image_mmcv(dataset_dict["file_name"], format=self.img_format)# read source image
        ...
        if "cam" in dataset_dict: # read image's camera intrinsic
        ...
        # CHW -> HWC
        coord_2d = get_2d_coord_np(im_W, im_H, low=0, high=1).transpose(1, 2, 0) # get every pixel's coordinate in source image in range [0,1]
        #######################################################################################
        # NOTE: currently assume flattened dicts for train
        assert self.flatten, "Only support flattened dicts for train now"
        inst_infos = dataset_dict.pop("inst_infos") # bbox info
        ...
        if cfg.MODEL.BBOX_TYPE.lower() == "visib":
            ...
            raise ValueError
        # bbox_center&scale: new roi center point and size in source image after add a nosiy
        # get new bbox's center point in source image and size after DZI
        # bbox_center, scale = self.aug_bbox_DZI(cfg, bbox_xyxy, im_H, im_W)
        bbox_center, scale = self.aug_bbox_DZI(cfg, bbox_xyxy, im_H, im_W, self.dzi_type, self.dzi_pad_scale, self.dzi_scale_ratio, self.dzi_shift_ratio)
        bw = max(bbox_xyxy[2] - bbox_xyxy[0], 1) # new bbox width
        ...
        ).transpose(2, 0, 1) # 以新bbox的中心为中心，将新bbox仿射变换到256*256

        # roi_img = self.normalize_image(cfg, roi_img)
        roi_img = self.normalize_image(cfg, roi_img, self.pixel_mean, self.pixel_std)
        ...
        ).transpose(2, 0, 1) # it is a 2D coordinate map that is 2D part in 2D-3D map
        ...
        dataset_dict["bbox_center"] = torch.as_tensor(bbox_center, dtype=torch.float32) # 新bbox中心在原图的坐标
        dataset_dict["scale"] = scale # 新bbox的尺寸
        dataset_dict["bbox"] = anno["bbox"]  # NOTE: original bbox
        dataset_dict["roi_wh"] = torch.as_tensor(np.array([bw, bh], dtype=np.float32)) # 新bbox的宽高
        dataset_dict["resize_ratio"] = resize_ratio = out_res / scale # 64与新bbox的尺寸的比值
        z_ratio = inst_infos["trans"][2] / resize_ratio
        obj_center = anno["centroid_2d"] # 3D model在2D图像中的投影
        delta_c = obj_center - bbox_center # 3D model中心在2D的投影坐标与新bbox中心坐标的差，用于计算位移的
        dataset_dict["trans_ratio"] = torch.as_tensor([delta_c[0] / bw, delta_c[1] / bh, z_ratio]).to(torch.float32) # 尺度不变位移（将位移全部除以长度，相当于取消了长度的影响）
        return dataset_dict
