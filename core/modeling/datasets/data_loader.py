...
class DatasetFromList(Base_DatasetFromList):
    ...
    def __init__(self, cfg, split, lst: list, copy: bool = True, serialize: bool = True, flatten=True):
        ...
        self.dzi_scale_ratio = try_get_key(cfg, "INPUT.DZI_SCALE_RATIO", "dzi_scale_ratio") 
        self.dzi_shift_ratio = try_get_key(cfg, "INPUT.DZI_SHIFT_RATIO", "dzi_shift_ratio")
    ...
    def read_data_test(self, dataset_dict):
        ...
        # noise = np.random.normal(0, cfg.TEST.GAUSSIAN_DEV, image.shape) #!
        # noise = np.random.normal(0, 80, image.shape) #!
        ...
        # CHW -> HWC
        coord_2d = get_2d_coord_np(im_W, im_H, low=0, high=1).transpose(1, 2, 0) # [(height, width, 2]
        #################################################################################
        ...
        for inst_i, inst_infos in enumerate(dataset_dict["annotations"]):
            ...
            roi_img = add_black_noise(roi_img, cfg.TEST.BLACK_NOISE_LEVEL)
            ...
            # roi_coord_2d
            roi_coord_2d = crop_resize_by_warp_affine( # 这个所谓的roi_coord_2d是2D-3D map中的2D部分
                coord_2d, bbox_center, scale, out_res, interpolation=cv2.INTER_LINEAR
            ).transpose(
                2, 0, 1
            )  # HWC -> CHW
...
def build_train_loader(cfg, dataset_names):
    ...
    # Images where none of the rois met the visible threshold were filtered out
    ...
    if cfg.MODEL.POSE_NET.XYZ_ONLINE:
        ...
        dataset = DatasetFromList(cfg, split="train", lst=dataset_dicts, copy=False)
...
def build_test_loader(cfg, dataset_name, train_objs=None, sampler=None, batch_size=1, pin_memory=False):
    dataset_dicts = get_detection_dataset_dicts(
        [dataset_name],
        filter_empty=False,
    ...
    dataset = DatasetFromList(cfg, split="test", lst=dataset_dicts, flatten=False)
