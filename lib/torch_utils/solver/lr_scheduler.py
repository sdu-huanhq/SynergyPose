...
def flat_and_anneal_lr_scheduler(
    ...
    if anneal_method == "step":
        ...
        anneal_start = anneal_point * total_iters # Calculate which iter starts anneal
    ...
    def f(x):  # x is the iter in lr scheduler, return the lr_factor
        ...
        if x < warmup_iters: # warmup
            if warmup_method == "linear": # lr = 1 when x = warmup_iters
        ...
        if x < anneal_start: # if iter is before anneal_start and warmup is conpleted, return 1
            return 1
        elif x >= anneal_start and x < total_iters: # anneal
            if anneal_method == "step":
    ...
    # torch.optim.lr_scheduler.LambdaLR can be able to adjust LR according to your own definition
    if return_function:
